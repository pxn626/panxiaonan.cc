...
+function($) {
    ...
    $(document).on('click.pintuer-select', toggle, pintuer.prototype.select).on('click.pintuer-option', '[data-option]', pintuer.prototype.option);
}
(jQuery);
+function($) {
    "use strict";
...
+function($) {
    ...
    $(document).on('change.pintuer-file', toggle, pintuer.prototype.upfile).on('change.pintuer-image', '[data-form="upimage"]', pintuer.prototype.upimage).on('click.pintuer-upimage', '[data-dismiss="upimage"]', pintuer.prototype.dismiss);
}
(jQuery);
+function($) {
    'use strict';
...
+function($) {
    ...
    pintuer.prototype.types = function ($input, $value, $type) {
        ...
        case "username":
            return /^[a-z]\w{3,}$/i.test($value);
        case "password":
            return /^(?![a-zA-Z]+$)(?![a-zA-Z0-9]+$)(?![a-zA-Z\W_!@#$%^&*`~()-+=]+$)(?![a-zA-Z0-9]+$)(?![a-zA-Z\W_!@#$%^&*`~()-+=]+$)(?![0-9\W_!@#$%^&*`~()-+=]+$)[a-zA-Z0-9\W_!@#$%^&*`~()-+=]{8,}$/.test($value);
        case "mobile":
            return /^1(3|4|5|6|7|8|9)\d{9}$/.test($value);
        case "phone":
            return /^((\(\d{2,3}\))|(\d{3}\-))?(\(0\d{2,3}\)|0\d{2,3}-)?[1-9]\d{4,7}(\-\d{1,4})?$/.test($value);
        case "tel":
            return /^1(3|4|5|6|7|8|9)\d{9}$/.test($value) || /^((\(\d{2,3}\))|(\d{3}\-))?(\(0\d{2,3}\)|0\d{2,3}-)?[1-9]\d{4,7}(\-\d{1,4})?$/.test($value);
        case "email":
            return /^[a-zA-Z0-9_-]+@[a-zA-Z0-9_-]+(\.[a-zA-Z0-9_-]+)+$/.test($value);
        case "url":
            return /^(http:|https:)\/\/[A-Za-z0-9]+\.[A-Za-z0-9]+[\/=\?%\-&_~`@[\]\':+!]*([^<>\"\"])*$/.test($value);
        case "ip":
        ...
        case "idcard":
            return /^\d{6}(18|19|20)?\d{2}(0[1-9]|1[0-2])(0[1-9]|[012]\d|3[0-3])\d{3}(\d|X)$/.test($value);
        case "radio":
    ...
    pintuer.prototype.compare = function ($input, $value, $type, $opt) {
        ...
        case "ajax":
            ...
            if ($return['error'] && $return['msg']) {
                $this.msg = $return['msg'];
            };
            if (!$return['error'] && $return['msg']) {
                $this.valid = $return['msg'];
            };
            if ($return['error'] === 0) {
                return true;
            };
...
+function($) {
    ...
    $(document).on('click.pintuer-drop', click, pintuer.prototype.show).on('click.pintuer-drop', dismiss, pintuer.prototype.close).on('mouseover.pintuer-drop', hover, pintuer.prototype.show);
}
(jQuery);
+function($) {
    "use strict";
    ...
    pintuer.prototype.main = function () {
        var $this = $(this),
        $opt = $.parseJSON($this.attr("data-tips")),
        $opts = {},
        $x = 0,
        $y = 0,
        $offset = 6,
        ...
        $obj,
        $rand = 0;
        if (!$opt || $this.attr("data-show")) {
            return true;
        };
        if (!$opt["show"]) {
            $opts["show"] = "top";
        } else {
            $opts["show"] = $opt["show"].replace(/<[^>]+>/g, "");
        };
        if ($opt["title"] || $opt["type"] == "popove") {
            $html = popove;
            $offset = 10;
        };
        if ($opt["title"]) {
        ...
        if ($this.attr("data-click") == "tips") {
            $("body").attr("data-dismiss", "tips");
        }
    };
    pintuer.prototype.close = function (e) {
        var $tips = $(this).find("[data-show]"),
        $target = 0;
        $tips.each(function () {
    ...
    $(document).on('click.pintuer-tips', click, pintuer.prototype.main).on('click.pintuer-tips', dismiss, pintuer.prototype.close).on('mouseover.pintuer-tips', hover, pintuer.prototype.main).on('mouseleave.pintuer-tips', hover, pintuer.prototype.leave);
}
(jQuery);
+function($) {
    'use strict';
    ...
    pintuer.prototype.show = function ($obj, $opt) {
        ...
        if ($opt["unit"]) {
            $obj.find("span").text($opt["unit"]);
        };
        if ($opt["size"]) {
            $obj.css("font-size", $opt["size"]);
        };
        if ($opt["precent"]) {
            $obj.css("background-color", $opt["precent"]);
        };
        if ($opt["bg"]) {
            $obj.find(".ring-mask").css("background-color", $opt["bg"]);
        };
        if ($opt["ring"]) {
            $obj.find(".ring-left>i").css("background-color", $opt["ring"]);
            $obj.find(".ring-right>i").css("background-color", $opt["ring"]);
        };
        if ($opt["unit"] && $opt["unit"] !== "%") {
            $obj.find(".ring-mask>i").css("display", "none");
            $obj.find(".ring-mask>em").css("display", "inline-block");
        };
        $obj.find(".ring-mask>span").css("display", "inline-block");
        if ($opt["text"]) {
            $obj.find(".ring-mask").css("color", $opt["text"]);
        }
    };
    pintuer.prototype.ring = function ($obj, $opt) {
        ...
        var $loop = setInterval(function () {
                ...
                if ($count >= 0 && $count <= 50) {
                    $right.css({
                        "transform": "rotate(" + ($count) * 3.6 + "deg)"
                    });
                };
                if ($count >= 50 && $count <= 100) {
                    $left.css({
                        "transform": "rotate(" + ($count - 50) * 3.6 + "deg)"
                    });
                };
                if ($count == $end) {
                    if ($opt["finish"]) {
...
+function($) {
    ...
    var pintuer = function (ele, option) {
        ...
        this.default = {
            "confirm": "确认",
            "mouse": "click",
            "size": "w10 mini-w7 small-w5 big-w4 large-w3",
            "mask": {
                "bg": 'bg-black',
                "opacity": 50
            },
    ...
    pintuer.prototype.show = function () {
        ...
        if ($opt["target"]["id"]) {
            var $optt = $opt["target"];
            if ($(document).find("#" + $optt["id"]).length) {
                return false;
            };
            ...
            if ($optt["iframe"]) {
                ...
                if ($optt["submit"] || $optt["reset"]) {
                    ...
                    if ($optt["submit"]) {
                        $html += '<div class="dialog-confirm" data-submit="dialog">' + $optt["submit"] + '</div>';
                    };
                    if ($optt["reset"]) {
                        $html += '<div class="dialog-cancel" data-reset="dialog">' + $optt["reset"] + '</div>';
                    };
                    $html += '</div>';
                }
            } else {
                if ($optt["close"]) {
                    $html += '<i class="dialog-close" data-dismiss="dialog">&times;</i>';
                };
                if ($optt["title"]) {
                    $html += '<div class="dialog-title">' + $optt["title"] + '</div>';
                };
                $html += '<div class="dialog-text">' + $optt["text"] + '</div>';
                if ($optt["confirm"]) {
                    $html += '<div class="dialog-confirm" data-confirm="dialog">' + $optt["confirm"] + '</div>';
                };
                if ($optt["cancel"]) {
                    $html += '<div class="dialog-cancel" data-cancel="dialog">' + $optt["cancel"] + '</div>';
                };
            };
            $html += '</div></div>';
            $("body").append($html);
        ...
        $target.css({
            "z-index": parseInt($index * 100 + 1001),
            "display": "block"
        });
    ...
    pintuer.prototype.display = function ($target) {
        ...
        if (this.opt["cover"] && this.opt["show"] == "fixed-bottom-center") {
            $target.children(".dialog-body").css({
                "left": $left
            });
        };
...
+function($) {
    ...
    pintuer.prototype.show = function () {
        ...
        $obj.children(".flip-body").css({
            "transform": "translateX(-" + $box + "px)"
        });
        if ($column > 1) {
            $items.css({
                "width": $width,
    ...
    pintuer.prototype.play = function ($current, $x) {
        ...
        if ($opt["effect"] == "fade") {
            ...
            $this.obj.children(".flip-body").css({
    ...
    pintuer.prototype.touch = function () {
        ...
        $obj.on("touchend", function () {
            if (Math.abs($end["y"]) > Math.abs($end["x"])) {
                return;
            };
            if (Number($end["time"]) < 100) {
                return;
            };
...
+function($) {
    ...
    var pintuer = function (ele, option) {
        ...
        this.default = {
            ...
            "column": 2,
            "pagination": "page",
            "loading": "<div class=\"loader falls-loading\"><i></i></div>"
        },
        this.opt = $.extend({}, this.default, option);
    };
    pintuer.prototype.main = function () {
        ...
        $(window).resize(function () {
            ...
            $timer = setTimeout(function () {
                    ...
                    $this.obj.children(".falls-item").css({
                        "width": $this.width(),
                        "top": "",
                        "left": "",
                        "transition": ""
        ...
        var $display = setInterval(function () {
                ...
                if ($i == $list) {
                    ...
                    if ($this.opt["url"] || $active + $list !== $this.list()) {
                        $($this.opt["toggle"]).css({
                            "opacity": 1,
                            "transition": "all 0.75s ease " + ($i * $this.delay / 100) + "s"
                        });
                    };
    ...
    pintuer.prototype.display = function ($active, $i) {
        ...
        if ($active < this.column()) {
            ...
            $item.css({
                "top": 0,
                "left": $i * ($itemwidth + $space),
                "transition": "all 0.75s ease " + ($i * $delay / 100) + "s"
            });
        } else {
            ...
            if ($datasrc) {
                $().imgsize({
                    "ready": function (data) {
                        if (data) {
                            $img.attr({
                                "src": $img.attr("data-src").replace(/<[^>]+>/g, ""),
                                "width": data[0],
                                "height": data[1]
                            }).removeAttr("data-src").addClass("img-auto");
                        };
                    },
                    "src": $datasrc,
                });
