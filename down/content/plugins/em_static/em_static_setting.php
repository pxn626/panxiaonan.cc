...
function em_static_setting_home() {
	em_static_update_url_config();
	em_static_update_config();
...
function em_static_setting_config() {
	if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		$GLOBALS['em_static_config_data']['enable_click_trace'] = isset($_POST['enable_click_trace']) ? intval($_POST['enable_click_trace']) : 0;		
		...
		$GLOBALS['em_static_config_data']['enable_debug_model'] = isset($_POST['enable_debug_model']) ? intval($_POST['enable_debug_model']) : 0;
		...
		$data .= '	"enable_click_trace" => '.$GLOBALS['em_static_config_data']['enable_click_trace'].','.chr(10);
		$data .= '	"enable_debug_model" => '.$GLOBALS['em_static_config_data']['enable_debug_model'].','.chr(10);
		$data .= ');';
...
function em_static_setting_url() {
	...
	if (empty($config) || ! is_array($config))
		$config = em_static_get_default_url_config();
	if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		$new_config = array();
		foreach ($config as $key => $v) {
			...
			if (preg_match('/path$/', $key)) {
				if ( ! empty($value) 
						&& ! preg_match('/^([\w-_\.\d\/]|\{%日志id%\}|\{%日志别名%\}|\{%页码%\}|\{%分类id%\}|\{%分类别名%\}|\{%标签别名%\}|\{%日期%\}|\{%用户id%\}|\{%页面id%\}|\{%页面别名%\})+$/', $value)) {
					$value = $v;
				}				
				...
				if ( ! preg_match('/^([\w-_\.\d]|\{%日志id%\}|\{%日志别名%\}|\{%页码%\}|\{%分类id%\}|\{%分类别名%\}|\{%标签别名%\}|\{%日期%\}|\{%用户id%\}|\{%页面id%\}|\{%页面别名%\})+\.html$/', $value)) {
					$value = $v;
				}
			}
			$new_config[$key] = $value;
		}
		$config_content = em_static_write_url_config_data($new_config);
