...
if ($GLOBALS['em_static_config_data']['enable_debug_model'] == 1) {
...
function em_static_create_start() {
	...
	$data['limits']['single_page_limit'] = isset($_POST['single_page_limit']) ? $_POST['single_page_limit'] : NULL;	
...
function em_static_create_single_page() {
	$data = em_static_get_data();
	$db = MySQL::getInstance();
	$limit = $data['limits']['single_page_limit'];
	$limit_start = isset($_GET['limit_start']) ? intval($_GET['limit_start']) : 0;
	$sql = 'SELECT gid, alias FROM '.DB_PREFIX.'blog WHERE hide = \'n\' AND type = \'page\' LIMIT '.$limit_start.','.$limit;
	$query = $db->query($sql);
	if ($db->num_rows($query) > 0) {
		$em_static = EMStatic::get_instance();
		$em_static->create_single_pages($limit_start, $limit);
		em_static_continue_create($data, "独立页面生成中。", $limit_start + $limit);				
	} else {
		em_static_go_to_next_step($data, '所有独立页面生成完毕');
	}
}
