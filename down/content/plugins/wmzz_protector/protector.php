...
function wmzz_prot_slog() { 
  if (WMZZ_PROT_LOG == 1) {
    $r = MySql::getInstance();
    $row=$r->once_fetch_array("SELECT * FROM `".DB_NAME."`.`".DB_PREFIX."options` WHERE `option_name` LIKE 'wmzz_prot_log'");
    $w = $row['option_value'] + 1;
    $r->query("UPDATE `".DB_NAME."`.`".DB_PREFIX."options` SET option_value = '{$w}' WHERE `option_name` LIKE 'wmzz_prot_log'");
  }
}
/**
 *  参数拆分
...
ob_clean(); flush();
$bname=Option::get('blogname'); //博客名称
?>
<!DOCTYPE html>
<html>
<head>			
<title>安全警告 - <?php echo $bname; ?></title>
<meta charset="utf-8" />
<style type="text/css">
body {
