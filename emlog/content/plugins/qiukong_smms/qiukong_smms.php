...
Plugin Name: SM图床
Version: 2
Description: 把图片上传到SM.MS
Author: qiukong.com
Author URL: http://qiukong.com
*/
if(!defined('EMLOG_ROOT')){die('err');}
addAction('adm_writelog_head',function(){
?>
<style>
#smms_pick{max-width:35%;}
#smms_list{margin:2px 0;font-size:12px;}
.smms_pic{margin:2px;width:136px;height:102px;display:inline-block;background-position:center;background-size:cover;background-repeat:no-repeat;}
.smms_act{padding-top:40px;box-sizing:border-box;width:100%;height:102px;background:rgba(0,0,0,0.2);text-align:center;}
...
<script>
function smms_cshow(node){
var item=document.createElement("div");
item.setAttribute("id","smms_body");
item.innerHTML="<input type=\"file\" onchange=\"smms_cpick()\" id=\"smms_pick\" multiple=\"multiple\" accept=\"image/*\" /><input type=\"button\" onclick=\"smms_cpush()\" id=\"smms_push\" value=\"开始\" /><input type=\"button\" onclick=\"smms_cpall()\" value=\"插入\" /><div id=\"smms_list\"></div>";
if(document.getElementById("smms_body")){node.parentNode.removeChild(document.getElementById("smms_body"));}else{node.parentNode.appendChild(item);};
};
...
for(var numb=0;numb<smms_item.length;numb++){if(smms_item[numb]){smms_html+='<p><a href="'+smms_item[numb].url+'" target="_blank"><img src="'+smms_item[numb].url+'" alt="'+smms_item[numb].filename+'" /></a></p>';};};
smms_cpave(smms_html);smms_html='';
};
var smms_pool=[],smms_item=[],smms_html='';
</script>
<span onclick="smms_cshow(this);" class="show_advset">SM图床</span>
<?php
});
?>